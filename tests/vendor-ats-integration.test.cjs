...
const dashboardStyles = fs.readFileSync(path.join(root, 'assets', 'vendor-dashboard.css'), 'utf8');
...
assert.match(dashboard, /REQUIREMENTS DASHBOARD/);
assert.match(dashboard, /Create Requirement/);
assert.match(dashboard, /Requirement Operations/);
assert.match(dashboard, /Requirement Management/);
assert.doesNotMatch(dashboard, />Jobs?</);
assert.match(dashboardStyles, /\.vendor-command-center/);
