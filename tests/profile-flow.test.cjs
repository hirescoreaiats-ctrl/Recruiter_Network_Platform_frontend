...
(async()=>{
 ...
 await page.locator('#profile-projects').waitFor();
 assert.equal(await page.locator('.candidate-quick-links').count(),0);
 assert.equal(await page.locator('.candidate-profile-columns > aside').count(),0);
 const widths=await page.locator('.candidate-profile-columns').evaluate(el=>({container:el.getBoundingClientRect().width,content:el.querySelector('main').getBoundingClientRect().width}));
 assert.ok(Math.abs(widths.container-widths.content)<2);
