...
candidateDashboard = async function () {
  ...
  try {
    ...
    if (grid && !document.querySelector('.chp-overview')) grid.insertAdjacentHTML('beforebegin', `<section class="chp-overview"><article><span class="purple">${candidateIcon('profile')}</span><div><small>Setup progress</small><b>${Math.round(readyCount / 3 * 100)}%</b><p>${matchingActive ? 'All required steps completed' : `${3 - readyCount} required step${3 - readyCount === 1 ? '' : 's'} incomplete`}</p></div></article><article><span class="blue">${candidateIcon('spark')}</span><div><small>Active requirements</small><b>${status.active_requirements_monitored || 0}</b><p>${status.active_requirements_monitored ? 'Live requirements in database' : 'No active requirements in database'}</p></div></article><article><span class="green">${candidateIcon('check')}</span><div><small>Verified matches</small><b>${portalList(status.matches).length}</b><p>${portalList(status.matches).length ? 'Live matches from your profile' : 'No matching opportunity in database'}</p></div></article></section>`);
