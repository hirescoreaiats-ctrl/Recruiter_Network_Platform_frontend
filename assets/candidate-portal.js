...
candidateDashboard = async function () {
  try {
    ...
    const profileReady = checks.profile ?? strength >= 70;
    const resumeReady = checks.resume ?? Boolean(profile.resume_file_id);
    const availabilityReady = checks.availability ?? Boolean(status.availability?.status);
