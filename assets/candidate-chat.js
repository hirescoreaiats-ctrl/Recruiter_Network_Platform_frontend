...
function initializeCandidateChat(panel, status) {
  ...
  panel.innerHTML = `<header><div><span class="cca-orb">✦</span><div><b>HireScoreAI Career Agent</b><small>Connected to your live candidate workspace</small></div></div><span class="cca-online"><i></i> Online</span></header><div class="cca-context"><span>${status.active_requirements_monitored || 0}<small>requirements monitored</small></span><span>${Object.values(status.profile_checks || {}).filter(Boolean).length}/3<small>profile signals ready</small></span><span>${candidateChatList(status.matches).length}<small>strong matches</small></span></div><div class="cca-thread" role="log" aria-label="Career Agent conversation" aria-live="polite"></div><div class="cca-prompts"><button type="button" data-agent-prompt="What should I complete next?">What should I complete next?</button><button type="button" data-agent-prompt="Scan my best recruiter matches">Scan my matches</button><button type="button" data-agent-prompt="Is my resume ready?">Check my resume</button></div><form class="cca-composer"><label for="career-message">Message your Career Agent</label><div><textarea id="career-message" rows="2" maxlength="500" placeholder="Ask about your profile, resume, availability or matches…"></textarea><button type="submit" aria-label="Send message" disabled><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 12h14m-6-6 6 6-6 6"/></svg></button></div><small>Responses use your authenticated profile and current recruiter requirements.</small></form>`;
