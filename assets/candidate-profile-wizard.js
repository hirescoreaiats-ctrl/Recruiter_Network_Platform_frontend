...
profilePage = async function () {
  ...
  try {
    ...
    const savedStrength = matchingStatus.profile_checks?.profile ? candidateProfileStrength(profile) : 0;
    ...
    layout(`<div class="candidate-job-portal cpw-page">
      ...
      </form></main><aside class="cpw-aside"><section><span>${candidateIcon('spark')}</span><h3>Why these details matter</h3><p>HireScoreAI matches structured evidence instead of guessing from keywords alone.</p><ul><li>✓ Better skill and title matching</li><li>✓ Correct experience context</li><li>✓ Country-aware compensation</li><li>✓ Eligibility and availability checks</li></ul></section><section class="cpw-strength"><div class="jp-completion-ring" style="--completion:${savedStrength * 3.6}deg"><span><b>${savedStrength}%</b><small>saved</small></span></div><h3>Saved profile strength</h3><p>${savedStrength ? 'This score reflects your last completed five-step profile.' : 'Complete and save all five steps to create your profile.'}</p></section></aside></div>
    </div>`, 'My Profile');
